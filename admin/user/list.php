...
<div class="card card-outline card-primary">
	<div class="card-header">
		<h3 class="card-title"><b><i class="nav-icon fas fa-users-cog"></i> Список пользователей системы </b></h3>
		<div class="card-tools">
			<a href="?page=user/manage_user" class="btn btn-flat btn-primary"><b><span class="fas fa-plus"></span> Создать нового пользователей: </b></a>
		</div>
	</div>
	<div class="card-body">
				...
				<tbody>
					...
					?>
						<tr>
							...
							<td class="text-center"><p class="m-0"><?php echo ($row['type'] == 1 )? "Администратор" : "Сотрудник" ?></p></td>
							<td align="center">
								 <button type="button" class="btn btn-default btn-sm dropdown-toggle dropdown-icon" data-toggle="dropdown"><i class="fas fa-exclamation-triangle"></i> Действие <span class="sr-only">Toggle Dropdown</span></button>
				                  <div class="dropdown-menu" role="menu">
									...
									<?php if($row['status'] != 1): ?>
				                    <a class="dropdown-item verify_user" href="javascript:void(0)" data-id="<?= $row['id'] ?>"  data-name="<?= $row['username'] ?>"><span class="fa fa-check text-primary"></span> Подтвердить </a>
				                    <div class="dropdown-divider"></div>
									<?php endif; ?>
...
<script>
	$(document).ready(function(){
		...
		$('.verify_user').click(function(){
			_conf("Вы уверены, чтобы подтвердить <b>"+$(this).attr('data-name')+"<b/>?","verify_user",[$(this).attr('data-id')])
		})
	})
